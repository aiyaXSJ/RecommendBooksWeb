				  ...
				  <el-col :span="6" v-for="(item,xxx)  in socialBook" >
						<el-card :body-style="{ padding: '15px' ,height:'400px'}">
						      ...
						      <div style="padding-top: 15px;">
						        <span style="font-size: 14px;"  @click="open(item.id,item.tag)">书名：{{ item.bookname }}</span>
...
<script>
	export default{
		...
		methods:{
			...
			async getBookLength(){
				// console.log("nihao")
				const result = await this.$axios.get("getOriginal");
				const data=result.data;
				this.total=data.length
			},
			async getBook(){
				const result = await this.$axios.get('getOriginalByC',{params:this.queryInfo});
				const data=result.data;
